      ...
      {isOpen && (
        <div className="fixed inset-0 bg-white dark:bg-slate-950/80 backdrop-blur-md flex items-center justify-center z-50 p-4">
          <motion.div
            ...
            className="bg-gray-50 dark:bg-slate-900 border border-gray-300 dark:border-slate-700/50 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden"
          >
            {/* Header Area with subtle gradient */}
            <div className="relative bg-gradient-to-r from-orange-600/20 to-red-600/10 p-6 border-b border-gray-300 dark:border-slate-700/50">
              <div className="flex justify-between items-start">
                ...
                <button
                  type="button"
                  onClick={onClose}
                  className="p-2 bg-white dark:bg-slate-800/50 hover:bg-gray-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white rounded-full transition-colors"
                >
                  <X size={18} />
                </button>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div>
                ...
                <div className="relative border-2 border-dashed border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-800/30 hover:bg-white dark:bg-slate-800/50 rounded-xl p-4 transition-colors group">
                  <input
                    type="file"
                    required
                    accept=".pdf,image/*"
                  ...
                  <div className="flex items-center gap-4 text-sm text-slate-700 dark:text-slate-300">
                    <div className="bg-gray-100 dark:bg-slate-700 p-3 rounded-lg group-hover:bg-slate-600 transition-colors">
                      <FileText size={20} className={proofFile ? "text-orange-400" : "text-slate-500 dark:text-slate-400"} />
                    </div>
                    <div className="flex-1 truncate">
                      {proofFile ? (
                        <span className="text-slate-900 dark:text-white font-medium">{proofFile.name}</span>
                      ) : (
                        <span className="text-slate-500 dark:text-slate-400">Click or drag PDF/Image here</span>
                      )}
                    </div>
                  </div>
                </div>
              ...
              {/* Reason Textarea */}
              <div>
                <label className="text-xs uppercase text-slate-500 dark:text-slate-400 font-bold mb-2 block">
                ...
                <textarea
                  ...
                  className="w-full bg-white dark:bg-slate-800/50 border border-gray-300 dark:border-slate-700 text-slate-900 dark:text-white p-3 rounded-xl focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500 min-h-[100px] resize-y text-sm transition-all placeholder:text-slate-600"
                ></textarea>
              </div>
              ...
              <div className="flex gap-4 pt-4 border-t border-gray-200 dark:border-slate-800">
                <button
                  type="button"
                  onClick={onClose}
                  className="flex-1 py-3 rounded-xl font-semibold bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-gray-100 dark:bg-slate-700 transition-colors"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="flex-1 py-3 rounded-xl font-bold bg-gradient-to-r from-orange-600 to-red-600 text-slate-900 dark:text-white hover:from-orange-500 hover:to-red-500 transition-all shadow-lg shadow-orange-900/50"
                >
